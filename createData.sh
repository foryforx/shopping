...
TOKEN=$(curl -X POST -F username=james -F password=james http://localhost:8080/login| awk -v  x=1 '{print $x}'| awk -F ":" '{print $7}'| awk -F '"' '{print $2}')
...
curl -i -X POST -H "Content-Type: application/json" -H "Authorization:Bearer $TOKEN"  -d "{ \"prodid\": 5, \"items\": 1 }" http://localhost:8080/auth/cart
curl -i -X POST -H "Content-Type: application/json" -H "Authorization:Bearer $TOKEN"  -d "{ \"prodid\": 6, \"items\": 4 }" http://localhost:8080/auth/cart
curl -i -X POST -H "Content-Type: application/json" -H "Authorization:Bearer $TOKEN"  -d "{ \"prodid\": 7, \"items\": 4 }" http://localhost:8080/auth/cart
