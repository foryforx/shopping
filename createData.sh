...
TOKEN=$(curl -X POST -F username=kal -F password=kal http://localhost:8080/login| awk -v  x=1 '{print $x}'| awk -F ":" '{print $7}'| awk -F '"' '{print $2}')
...
# # DELETE cart item
# curl -i -X GET -H "Content-Type: application/json" -H "Authorization:Bearer $TOKEN" http://127.0.0.1:8080/auth/cart
# curl -i -X DELETE  -H "Authorization:Bearer $TOKEN" http://localhost:8080/auth/cart?id=4
# curl -i -X GET -H "Content-Type: application/json" -H "Authorization:Bearer $TOKEN" http://127.0.0.1:8080/auth/cart
